...
function hit(s:State,owner:number,a:any,x:number,face:number,projectile=false){
 ...
 const blocking=!!(t.lastMask&2)&&(!t.stun||t.action==='BLOCKSTUN_ACTION_DEFINITION')&&(t.action===IDLE||t.action==='BLOCKSTUN_ACTION_DEFINITION')
...
export function step(previous:State,inputs:Input[],mode:'play'|'move'='play'):State{
 ...
 for(let i=0;i<2;i++){
  ...
  if(c){change(f,c.m_nCancelActionID);f.history=[];a=action(f)}
  if(f.action===IDLE){
